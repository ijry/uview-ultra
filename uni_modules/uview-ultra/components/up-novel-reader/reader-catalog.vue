<template>
    <view class="up-novel-reader__catalog" :style="catalogStyle">
        ...
        <scroll-view v-if="chapters.length" class="up-novel-reader__catalog-scroll" scroll-y>
            <view
            ...
            >
                ...
                <text class="up-novel-reader__catalog-item-title">
                    {{ chapter.title || `第${displayIndex(chapter)}章` }}
                </text>
                <up-icon v-if="chapter.isLocked" name="lock" size="15" :color="themeTokens.muted"></up-icon>
            </view>
            <view v-if="bookmarks.length" class="up-novel-reader__bookmark-section">
                ...
                >
                    ...
                    <text class="up-novel-reader__bookmark-progress">
                        {{ Number(bookmark.chapterIndex || 0) + 1 }} 章
                    </text>
                </view>
            </view>
        </scroll-view>
...
export default {
    ...
    methods: {
        displayIndex(chapter) {
            return Number(chapter && chapter.index) + 1
        },
        isCurrent(chapter) {
            return Boolean(
                chapter &&
