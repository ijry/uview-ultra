...
const pageSizeIndex = computed(() => {
	const index = props.pageSizes.findIndex(size => size.value === props.pageSize)
	return index >= 0 ? index : 0
})
const pageSizeLabel = computed(() => {
	const found = props.pageSizes.find(size => size.value === props.pageSize)
	return found?.label || props.pageSize
})
...
function handleSizeChange(e) {
	...
	const size = props.pageSizes[selected]?.value || props.pageSizes[0].value
...
function onInputPage(e) {
	currentPageInput.value = e.detail.value
}
function onConfirmPage(e) {
