...
const elIndex = ref(guid())

// 由于一些特殊原因，不能将此变量放到 data 中定义
let observer = {}
let contentObserver = null

// 将threshold从rpx转为px
const getThreshold = computed(() => {
	// 先取绝对值，因为threshold可能是负数，最后根据 props.threshold 是正数或者负数，重新还原
	const thresholdPx = uni.upx2px(Math.abs(props.threshold))
...
function disconnectObserver(observerName) {
	...
	const target = observerName === 'observer' ? observer : null
	target && target.disconnect && target.disconnect()
}
...
onMounted(() => {
	...
	setTimeout(() => {
		...
		}).observe('.up-lazy-item-' + elIndex.value, (res) => {
